...
summary.rpcmfit <- function(x) {
  ...
  item.names <- names(x$item_params)
  cat(paste0("Parameter Estimation: \n"))
  for(i in 1:length(x$item_params))
    {
      cat(paste0("  ", item.names[i], ": ", x$item_params[i], "\n"))
    }


  if(x$engine == "em"){
    cat(paste0("Maximum Iteration: ", x$estimation[8], "\n"),
        paste0("Iterations: ", x$estimation[7], "\n"))
  }
